        ...
        if ($imageIdsForDelete) {
            foreach ($currentImages as $currentImage) {
                if (in_array($currentImage->id, $imageIdsForDelete)) {
                    ...
                    Storage::disk('public')->delete(str_replace('images/', 'images/preview/prev_', $currentImage->path));
                    $currentImage->delete();
                }
            }
        }
        ...
        if ($images) {
            foreach ($images as $image) {
                ...
                Image::create([
                    ...
                    'preview_url' => url('/storage/images/preview/' . $previewName),
                    'post_id' => $post->id
                ]);

                \Intervention\Image\Facades\Image::make($image)
                    ->fit(100, 100)
                    ->save(storage_path('app/public/images/preview/' . $previewName));
            }
        }
